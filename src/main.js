...
const notify = (time, type) => {
  if (time === "00:01") {
    var notification = new Notification({
      title: "Pomdor",
      body: type == "pause" || type == "longpause" ? "🍅 Pomodor" : "🍌 Pause",
      silent: true,
      icon: icon
    });
    notification.show();
...
function update() {
  let date = moment();
  let timer = null;
  if (disabled || date.day() === 0 || date.day() === 6) {
    setNull();
  ...
  if (luxaforId && type !== status) {
    ...
    if (type === "longpause") {
      if (programPaths.length > 0)
        programPaths.forEach(p => {
          console.log("path", p);
          let program = spawn(p.path);
          program.on("close", (code, signal) => {
            console.log(
              `${p.name} process terminated due to receipt of signal ${signal}`
            );
          });
